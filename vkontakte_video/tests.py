...
class CommentTest(TestCase):
    ...
    def test_comment_crud_methods(self):
        ...
        # create
        comment = Comment(text='Test comment', video=video, author=owner, date=timezone.now())
        ...
        comment = Comment.objects.create(
            text='Test comment created by manager', video=video, author=owner, date=timezone.now(), commit_remote=True)
        self.objects_to_delete += [comment]
    ...
    def test_parse_comment(self):
        ...
        self.assertEqual(instance.remote_id, '-%s_811' % GROUP_ID)
